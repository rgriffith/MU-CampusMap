...
(document.getElementById(a.layers.bldgLayers[b]).checked=!1,a.removeKmlLayer(a.layers.bldgLayers[b]))}else a.removeKmlLayer(this.id)})},resizeMap:function(){google.maps.event.trigger(this.map,"resize")},resetMap:function(){this.map.panTo(this.defaults.center);this.map.setZoom(this.defaults.zoom)},addMarker:function(a){var b=this,c=a.toJSON(),d=c.marker;d.setMap(this.map);google.maps.event.addListener(d,"click",function(){var e=new google.maps.InfoWindow({content:b.infoWindowTemplate({marker:c}),position:d.getPosition()});
google.maps.event.addListener(e,"closeclick",function(){a.set({showInfowindow:!1})});a.set({showInfowindow:!0});b.setInfoWindow(e);e.open(b.map,d)})},openMarkerInfowindow:function(a){var b=a.get("marker");!0===a.get("showInfowindow")&&null!==b.getMap()?google.maps.event.trigger(b,"click"):this.setInfoWindow(null)},setInfoWindow:function(a){null!==this.infowindow&&this.infowindow.close();this.infowindow=a},showKmlLayer:function(a,b){void 0===this.layers.kml[a]&&(this.layers.kml[a]=new google.maps.KmlLayer(b.url+
"?v="+this.kmlVersion,{suppressInfoWindows:b.suppressInfoWindows,preserveViewport:b.preserveViewport}));this.layers.kml[a].setMap(this.map);this.addKMLLayerClickEvent(a)},addKMLLayerClickEvent:function(a){var b=this;("baseLayer"===a||-1!==_.indexOf(b.layers.bldgLayers,a,!0)||-1!==_.indexOf(b.layers.infoLayers,a,!0))&&google.maps.event.addListener(b.layers.kml[a],"click",function(a){a=new google.maps.InfoWindow({content:a.featureData.infoWindowHtml,position:new google.maps.LatLng(a.latLng.lat(),a.latLng.lng())});
b.setInfoWindow(a);a.open(b.map)})},removeKmlLayer:function(a){this.layers.kml[a].setMap(null)}});window.PanelView=Backbone.View.extend({el:$("#panel-wrapper"),statsTemplate:_.template($("#search-results-stats").html()),resultsTemplate:_.template($("#search-results-item").html()),shareFieldsTemplate:_.template($("#share-fields").html()),searchResults:[],searchOpts:{curPage:1,totalPages:1,pageSize:6},routeUrl:"",events:{},initialize:function(){var a=this;this.input=this.$("#kwsearch-keyword");this.selectbox=
this.$("#bldgsearch-select");this.deepLinksIds=[];$("#options-nav-bar").find("a:first").addClass("selected");$("#map-search").show();0===a.searchResults.length&&$("#map-overlays").show();$("#options-nav-bar").on("click","a.btn",function(){$("#options-nav-bar").find("a").removeClass("active");$(this).addClass("active");$("fieldset",$("#marker-search")).hide();$("#sharedlink").blur();switch(this.id){case "tablink-mapoptions":$("#map-search, #link").hide();$("#map-overlays").show();break;case "tablink-link":$("#map-search, #map-overlays").hide();
...
"change select#bldgsearch-select":"searchByBuilding"},initialize:function(){var a=this;this.Collections.Locations=new LocationCollection;this.Collections.Locations.fetch();this.Router=new AppRouter;this.Views.CampusMap=new MapView({collection:this.Collections.Locations,router:this.Router});this.Views.Panel=new PanelView({collection:this.Collections.Locations,router:this.Router});this.Views.Panel.routeUrl=this.getRouteUrl();this.Views.Panel.updateShareFields();var b=function(b,d){d=parseFloat(d)||
1;b=unescape(b.replace("+"," "));""!==b?(a.Collections.Locations.removeShownMarkers(),a.trackVirtualPageView(),_.delay(function(){a.Views.Panel.routeUrl=a.getRouteUrl();a.Views.Panel.searchByKeyword(b,d);a.Views.Panel.updateShareFields()},300)):a.clearResults()};this.Router.route("","",function(){a.Collections.Locations.removeShownMarkers();a.Views.Panel.routeUrl="";a.Views.Panel.clearResults();a.trackVirtualPageView();a.Views.Panel.updateShareFields();a.Views.CampusMap.resetMap();return!1});this.Router.route("search/:query",

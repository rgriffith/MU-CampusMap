$(function(){
	...
	window.LocationCollection = Backbone.Collection.extend({
		model: Location,
		url: "lib/data/json/mu_campusmap_locations.json",
		...
		// Search the collection for a supplied system id.
		// ---------------
		searchBySystemId: function(systemId) {
			return this.filter(function(loc) {
				var pattern = new RegExp(systemId, 'i');
				return pattern.test(loc.get('systemid'));
			});
		},
	...
	window.MapView = Backbone.View.extend({	
		...
		initialize: function() {
			...
			// Round the current time to 30 minute intervals for KML caching.
			theDate.setMinutes((this.defaults.timestampInterval * Math.floor(theDate.getMinutes() / this.defaults.timestampInterval)));
			this.kmlTimestamp = theDate.getTime();
		...
		resizeMap: function() {
			...
			if (this.infowindow !== null) {
				this.map.panTo(this.infowindow.position);
			} else {
		...
		addKmlLayer: function(layerId, kmlOptions) {
			...
			if (this.layers.kml[layerId] === undefined) { 
				this.layers.kml[layerId] = new google.maps.KmlLayer(kmlOptions.url + '?v=' + this.kmlTimestamp, {
		...
		addKMLLayerClickEvent: function(layerId) {
			...
			if (layerId === 'baseLayer' || _.indexOf(self.layers.bldgLayers, layerId, true) !== -1 || _.indexOf(self.layers.infoLayers, layerId, true) !== -1) {			
				google.maps.event.addListener(self.layers.kml[layerId], "click", function(event) { 
					...
					self.map.panTo(infowindow.getPosition())
		       });
		   }
		}
	});
	...
	window.AppView = Backbone.View.extend({
		...
		modalTemplate: _.template($('#marker-modal-content').html()),
		...
		events: {
			...
			"click a[class^='modal-toggler']": "toggleModalWindow"		
		},
		...
		toggleKmlLayer: function(evt) {
			...
			if (checkbox.checked === true) {
				...
				if (checkbox.id !== 'baseLayer' && isBuilding && document.getElementById('baseLayer').checked === true) {
					...
					// building layers.
					var i = 0;
					for (; i < self.Views.CampusMap.layers.bldgLayers.length; i++) {
						if (document.getElementById(self.Views.CampusMap.layers.bldgLayers[i]).checked == true) {
							document.getElementById(self.Views.CampusMap.layers.bldgLayers[i]).checked = false;
		...
		toggleModalWindow: function(evt) {
			var self = this,
				link = evt.currentTarget,
				markerId = evt.currentTarget.className.replace('modal-toggler-', ''),
				marker = self.Collections.Locations.get(markerId);
			
			// If we couldn't find the marker by 1-up ID, try searching by systemid (Cascade Page ID).
			if (marker === undefined) {
				marker = self.Collections.Locations.searchBySystemId(markerId)[0];
			}
			
			if (marker !== undefined) {
				$('#modalContain').html(self.modalTemplate({marker: marker.toJSON()}));

$(function(){
	...
	window.MapView = Backbone.View.extend({	
		...
		initialize: function() {
			var self = this,
				baseKmlUrl = 'http://www.millersville.edu/dev/directions/data/markers.kml';
			...
			$('#filter-map').on('change', 'input.filter-checkbox', function(){
				...
				if (checkbox.checked === true) {
					var isBuilding = _.indexOf(self.layers.bldgLayers, checkbox.id, true) !== -1;
					...
					if (checkbox.id !== 'baseLayer' && isBuilding && document.getElementById('baseLayer').checked === true) {
						...
						for (var i = 0; i < self.layers.bldgLayers.length; i++) {
							if (document.getElementById(self.layers.bldgLayers[i]).checked == true) {
								...
								self.removeKmlLayer(self.layers.bldgLayers[i]);
							}
						}
					}
				} else {
	...
	window.AppView = Backbone.View.extend({
		...
		togglePanelDisplay: function() {
			...
			if (offset.left < 0 ) {
				...
				$("#options-nav-bar").animate({left: "22px", height: '10px', padding: '5px'}, 100, "linear", function(){
					// Switch the arrow's orientation and title.
					$('#map-options-toggler').attr({

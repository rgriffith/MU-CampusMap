...
this.layers.kml.baseLayer=new google.maps.KmlLayer(b,{suppressInfoWindows:!1,preserveViewport:!0});this.layers.kml.baseLayer.setMap(this.map);$("#filter-map").on("change","input.filter-checkbox",function(){var b=["academicsLayer","administrativeLayer","dormLayer"];if(!0==this.checked){var c=-1!=this.value.indexOf("building")?!0:!1;a.showKmlLayer(this.id,{url:"http://www.millersville.edu/directions/kml/mobile/"+this.value+".kml?v="+a.kmlVersion,suppressInfoWindows:c,preserveViewport:!0});if("baseLayer"!=
this.id&&c&&!0==document.getElementById("baseLayer").checked)document.getElementById("baseLayer").checked=!1,a.removeKmlLayer("baseLayer");else if("baseLayer"==this.id)for(c=0;c<b.length;c++)!0==document.getElementById(b[c]).checked&&(document.getElementById(b[c]).checked=!1,a.removeKmlLayer(b[c]))}else a.removeKmlLayer(this.id)})},resizeMap:function(){google.maps.event.trigger(this.map,"resize")},addMarker:function(a){var b=this,d=a.toJSON(),c=d.marker;c.setMap(this.map);google.maps.event.addListener(c,
"click",function(){var e=new google.maps.InfoWindow({content:d.infoWindow.content,position:c.getPosition()});google.maps.event.addListener(e,"closeclick",function(){a.set({showInfowindow:!1})});b.setInfoWindow(e);e.open(b.map,c)})},openMarkerInfowindow:function(a){var b=a.get("marker");!0===a.get("showInfowindow")&&null!==b.getMap()?google.maps.event.trigger(b,"click"):this.setInfoWindow(null)},setInfoWindow:function(a){null!==this.infowindow&&this.infowindow.close();this.infowindow=a},showKmlLayer:function(a,
...
a.trackVirtualPageView();a.Views.Panel.updateShareFields();return!1});this.Router.route("search/:query","search",b);this.Router.route("search/:query/p:page","search",b);this.Router.route("locations/:ids","locations",function(b){b=b.split(",");a.Collections.Locations.removeShownMarkers();a.trackVirtualPageView();_.delay(function(){a.Views.Panel.routeUrl=a.getRouteUrl();a.Views.Panel.searchByIDs(b);a.Views.Panel.updateShareFields()},300)});Backbone.history.start()},searchByKeyword:function(a){a.preventDefault();
this.Router.navigate("search/"+escape($("#kwsearch-keyword").val()),!0)},searchByBuilding:function(a){a.preventDefault();this.Router.navigate("locations/"+escape(a.currentTarget.value),!0)},paginate:function(a){a=parseInt(a.currentTarget.innerHTML);if("number"!==typeof a||isNaN(a))a=1;this.Views.Panel.updateShareFields();this.trackVirtualPageView();this.Views.Panel.updateResults(a)},getRouteUrl:function(){var a=""!==location.hash?location.hash.match(/^#([a-z0-9,%-]*\/?){2}/)[0]:"#";return"/"!==a.charAt(a.length-
1)?a+"/":a},clearResults:function(){this.Router.navigate("",!0)},trackVirtualPageView:function(){_gaq&&"object"===typeof _gaq&&_gaq.push(["_trackPageview",location.pathname+location.search+location.hash])},togglePanelDisplay:function(){var a=this;0>this.Views.Panel.$("#features-panel").offset().left?($("#features-panel, #map-canvas").animate({left:"0px"},100,"linear"),$("#options-nav-bar").animate({left:"0px",height:"29px",top:"0px",padding:"15px"},100,"linear",function(){$("#map-options-toggler").attr("title",
"Close this sidebar").css({backgroundPosition:"48% 4px",top:"17px",right:"10px"})})):($("#features-panel, #map-canvas").animate({left:"-340px"},100,"linear",function(){a.Views.CampusMap.resizeMap()}),$("#options-nav-bar").animate({left:"-300px",height:"10px",padding:"5px"},100,"linear",function(){$("#map-options-toggler").attr("title","Open this sidebar").css({backgroundPosition:"48% -28px",top:"-1px",right:"-2px"})}));return!1}});window.App=new AppView});
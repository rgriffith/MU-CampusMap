...
return!1});this.Router.route("search/:query","search",b);this.Router.route("search/:query/p:page","search",b);this.Router.route("locations/:ids","locations",function(b){b=b.split(",");a.Collections.Locations.removeShownMarkers();_.delay(function(){a.Views.Panel.routeUrl=a.getRouteUrl();a.Views.Panel.searchByIDs(b);a.Views.Panel.updateShareFields()},300)});Backbone.history.start()},searchByKeyword:function(a){a.preventDefault();this.Router.navigate("search/"+escape($("#kwsearch-keyword").val()),!0)},
searchByBuilding:function(a){a.preventDefault();this.Router.navigate("locations/"+escape(a.currentTarget.value),!0)},paginate:function(a){a=parseInt(a.currentTarget.innerHTML);if("number"!==typeof a||isNaN(a))a=1;this.Views.Panel.updateShareFields();this.Views.Panel.updateResults(a)},getRouteUrl:function(){var a=""!==location.hash?location.hash.match(/^#([a-z0-9,%-]*\/?){2}/)[0]:"#";return"/"!==a.charAt(a.length-1)?a+"/":a},clearResults:function(){this.Router.navigate("",!0)},togglePanelDisplay:function(){var a=
this;0>this.Views.Panel.$("#features-panel").offset().left?($("#features-panel, #map-canvas").animate({left:"0px"},100,"linear"),$("#options-nav-bar").animate({left:"0px",height:"29px",top:"0px",padding:"15px"},100,"linear",function(){$("#map-options-toggler").attr("title","Close this sidebar").css({backgroundPosition:"48% 4px",top:"17px",right:"10px"})})):($("#features-panel, #map-canvas").animate({left:"-340px"},100,"linear",function(){a.Views.CampusMap.resizeMap()}),$("#options-nav-bar").animate({left:"-300px",
height:"10px",padding:"5px"},100,"linear",function(){$("#map-options-toggler").attr("title","Open this sidebar").css({backgroundPosition:"48% -28px",top:"-1px",right:"-2px"})}));return!1}});window.App=new AppView});